...
`make run` opens a real window. It needs a display; in a headless session it fails at
`QApplication`, which is expected and not a bug to fix.
...
- **Collectors must never raise.** Every source is optional and may be absent; each
  reader returns a default-constructed metrics object on failure so the HUD degrades
  to zeros instead of dying.
- **One network hop only**, the GitHub commit calendar, at `COMMITS_INTERVAL_MS`
